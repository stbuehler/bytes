...
fn alloc_small(b: &mut Bencher) {
    b.iter(|| {
        for _ in 0..1024 {
            test::black_box(UnBytesMut::with_capacity(12));
        }
    })
}
...
fn alloc_mid(b: &mut Bencher) {
    b.iter(|| {
        test::black_box(UnBytesMut::with_capacity(128));
    })
}
...
fn alloc_big(b: &mut Bencher) {
    b.iter(|| {
        test::black_box(UnBytesMut::with_capacity(4096));
    })
}
...
fn deref_unique(b: &mut Bencher) {
    let mut buf = UnBytesMut::with_capacity(4096);
    buf.put(&[0u8; 1024][..]);
...
fn deref_inline(b: &mut Bencher) {
    let mut buf = UnBytesMut::with_capacity(8);
    buf.put(&[0u8; 8][..]);
...
fn deref_two(b: &mut Bencher) {
    let mut buf1 = UnBytesMut::with_capacity(8);
    buf1.put(&[0u8; 8][..]);

    let mut buf2 = UnBytesMut::with_capacity(4096);
    buf2.put(&[0u8; 1024][..]);
...
fn alloc_write_split_to_mid(b: &mut Bencher) {
    b.iter(|| {
        let mut buf = UnBytesMut::with_capacity(128);
...
fn drain_write_drain(b: &mut Bencher) {
    ...
    b.iter(|| {
        let mut buf = UnBytesMut::with_capacity(1024);
...
fn slice_empty(b: &mut Bencher) {
    b.iter(|| {
        let b = UnBytes::from(vec![17; 1024]).clone();
        for i in 0..1000 {
