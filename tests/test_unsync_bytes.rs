...
use bytes::{unsync::UnBytes, unsync::UnBytesMut, BufMut};
...
fn is_not_sync<T/*: !Sync*/>() {}
fn is_not_send<T/*: !Send*/>() {}

#[test]
fn test_bounds() {
    is_not_sync::<UnBytes>();
    is_not_sync::<UnBytesMut>();
    is_not_send::<UnBytes>();
    is_not_send::<UnBytesMut>();
}

#[test]
fn from_slice() {
    let a = UnBytes::from(&b"abcdefgh"[..]);
    ...
    let a = UnBytesMut::from(&b"abcdefgh"[..]);
    assert_eq!(a, b"abcdefgh"[..]);
    assert_eq!(a, &b"abcdefgh"[..]);
    assert_eq!(a, Vec::from(&b"abcdefgh"[..]));
    assert_eq!(b"abcdefgh"[..], a);
...
#[test]
fn fmt() {
    let a = format!("{:?}", UnBytes::from(&b"abcdefg"[..]));
    let b = "b\"abcdefg\"";

    assert_eq!(a, b);

    let a = format!("{:?}", UnBytesMut::from(&b"abcdefg"[..]));
    assert_eq!(a, b);
}
...
fn fmt_write() {
    ...
    let mut a = UnBytesMut::with_capacity(64);
    write!(a, "{}", &s[..64]).unwrap();
    assert_eq!(a, s[..64].as_bytes());


    let mut b = UnBytesMut::with_capacity(64);
    write!(b, "{}", &s[..32]).unwrap();
    write!(b, "{}", &s[32..64]).unwrap();
    assert_eq!(b, s[..64].as_bytes());


    let mut c = UnBytesMut::with_capacity(64);
...
#[test]
fn len() {
    let a = UnBytes::from(&b"abcdefg"[..]);
    assert_eq!(a.len(), 7);

    let a = UnBytesMut::from(&b"abcdefg"[..]);
    assert_eq!(a.len(), 7);

    let a = UnBytes::from(&b""[..]);
    assert!(a.is_empty());

    let a = UnBytesMut::from(&b""[..]);
    assert!(a.is_empty());
}

#[test]
fn index() {
    let a = UnBytes::from(&b"hello world"[..]);
...
#[test]
fn split_off() {
    let mut hello = UnBytes::from(&b"helloworld"[..]);
    let world = hello.split_off(5);

    assert_eq!(hello, &b"hello"[..]);
    assert_eq!(world, &b"world"[..]);

    let mut hello = UnBytesMut::from(&b"helloworld"[..]);
    let world = hello.split_off(5);
...
fn split_off_uninitialized() {
    let mut bytes = UnBytesMut::with_capacity(1024);
...
fn split_off_to_loop() {
    ...
    for i in 0..(s.len() + 1) {
        ...
        {
            let mut bytes = UnBytesMut::from(&s[..]);
            let off = bytes.split_off(i);
            assert_eq!(i, bytes.len());
            let mut sum = Vec::new();
            sum.extend(&bytes);
...
fn split_off_to_at_gt_len() {
    fn make_bytes() -> UnBytes {
        let mut bytes = UnBytesMut::with_capacity(100);
...
fn fns_defined_for_bytes_mut() {
    let mut bytes = UnBytesMut::from(&b"hello world"[..]);
...
fn reserve_convert() {
    // Inline -> Vec
    let mut bytes = UnBytesMut::with_capacity(8);
    bytes.put("hello");
    bytes.reserve(40);
    ...
    // Inline -> Inline
    let mut bytes = UnBytesMut::with_capacity(inline_cap());
    ...
    // Vec -> Vec
    let mut bytes = UnBytesMut::from(LONG);
    bytes.reserve(64);
...
#[test]
fn reserve_growth() {
    let mut bytes = UnBytesMut::with_capacity(64);
...
fn reserve_max_original_capacity_value() {
    ...
    let mut bytes = UnBytesMut::with_capacity(SIZE);
...
fn reserve_in_arc_unique_does_not_overallocate() {
    let mut bytes = UnBytesMut::with_capacity(1000);
    bytes.take();
...
#[test]
fn extend_mut() {
    let mut bytes = UnBytesMut::with_capacity(0);
    bytes.extend(LONG);
...
#[test]
fn from_static() {
    let mut a = UnBytes::from_static(b"ab");
    let b = a.split_off(1);
...
#[test]
fn advance_inline() {
    let mut a = UnBytes::from(&b"hello world"[..]);
    a.advance(6);
    assert_eq!(a, &b"world"[..]);
}
...
#[test]
fn advance_static() {
    let mut a = UnBytes::from_static(b"hello world");
    a.advance(6);
    assert_eq!(a, &b"world"[..]);
}
...
#[test]
fn advance_vec() {
    let mut a = UnBytesMut::from(b"hello world boooo yah world zomg wat wat".to_vec());
    a.advance(16);
...
#[should_panic]
fn advance_past_len() {
    let mut a = UnBytesMut::from(b"hello world".to_vec());
    a.advance(20);
}

#[test]
fn partial_eq_bytesmut() {
    let bytes = UnBytes::from(&b"The quick red fox"[..]);
    let bytesmut = UnBytesMut::from(&b"The quick red fox"[..]);
    assert!(bytes == bytesmut);
    assert!(bytesmut == bytes);
    let bytes2 = UnBytes::from(&b"Jumped over the lazy brown dog"[..]);
...
fn bytes_unsplit_basic() {
    let mut buf = UnBytes::with_capacity(64);
...
fn bytes_unsplit_empty_self() {
    // empty self
    let mut buf = UnBytes::new();

    let mut other = UnBytes::with_capacity(64);
...
fn bytes_unsplit_inline_arc() {
    let mut buf = UnBytes::with_capacity(8); //inline
    buf.extend_from_slice(b"aaaabbbb");

    let mut buf2 = UnBytes::with_capacity(64);
...
fn bytes_unsplit_arc_inline() {
    ...
    let mut buf2 = UnBytes::with_capacity(8); //inline
...
fn bytes_unsplit_both_inline() {
    let mut buf = UnBytes::with_capacity(16); //inline
...
fn bytes_mut_unsplit_basic() {
    let mut buf = UnBytesMut::with_capacity(64);
    buf.extend_from_slice(b"aaabbbcccddd");
...
fn bytes_mut_unsplit_empty_self() {
    // empty self
    let mut buf = UnBytesMut::new();

    let mut other = UnBytesMut::with_capacity(64);
    other.extend_from_slice(b"aaabbbcccddd");
...
fn bytes_mut_unsplit_inline_arc() {
    let mut buf = UnBytesMut::with_capacity(8); //inline
    buf.extend_from_slice(b"aaaabbbb");

    let mut buf2 = UnBytesMut::with_capacity(64);
    buf2.extend_from_slice(b"ccccddddeeee");
...
fn bytes_mut_unsplit_arc_inline() {
    ...
    let mut buf2 = UnBytesMut::with_capacity(8); //inline
    buf2.extend_from_slice(b"ccccdddd");
...
fn bytes_mut_unsplit_both_inline() {
    let mut buf = UnBytesMut::with_capacity(16); //inline
    buf.extend_from_slice(b"aaaabbbbccccdddd");
